...
`.github/workflows/ci.yml` runs 12 jobs in parallel on every push/PR to
`main`, all on GitHub-hosted runners: `workspace-lint` (fmt + clippy) and
`workspace-test` (nextest + doctests + the API route table), `drift` (TS wire
...
- **All workflows, including image builds and nightly benches, run on
  GitHub-hosted runners** (standard `ubuntu-latest` minutes, free on a public
  repo).
