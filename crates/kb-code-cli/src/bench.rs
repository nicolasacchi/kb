...
//! is scored: a HIT at rank k if `expect_path` equals some hit's `path`
//! field within the first k results (1-based; lanes whose hits carry no
//! `path` field, e.g. sessions/transcripts, never match). recall@1/@5 are
...
/// `None` if absent. Only hits carrying a `path` field are ever
/// considered.
pub fn hit_rank(results: &[serde_json::Value], expect_path: &str) -> Option<usize> {
    results
        .iter()
        .position(|h| h.get("path").and_then(|v| v.as_str()) == Some(expect_path))
        .map(|i| i + 1)
}
...
mod tests {
    ...
    #[test]
    fn hit_rank_ignores_hits_with_no_path_field() {
        let results = vec![serde_json::json!({"session_id": "s1"})];
        assert_eq!(hit_rank(&results, "s1"), None);
    }
