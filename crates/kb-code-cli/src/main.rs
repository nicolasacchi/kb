...
enum Cmd {
    ...
    /// `kb-code comments <SUB> --repo R` — V72-J1: the `comments/1` index
    /// (`GET /api/comments*`). One scanner over every comment in the repo,
    /// classified doc | annotation | directive | section | licence |
    /// generated | commented_code | prose, with a per-request drift oracle
    /// that reports arithmetic and never a verdict. `kb-code todos` is the
    /// narrow, byte-compatible view over the same rows. Daemon-only.
    Comments {
        #[command(subcommand)]
        cmd: CommentsCmd,
    },
...
/// `kb-code comments …` — V72-J1's `comments/1` read family. Every verb
/// is a pure READ; nothing here mutates source (D8's rule: a change to
/// comment text rides the EXISTING suggestion + apply path, never a new
/// mutation route).
#[derive(Subcommand, Debug)]
enum CommentsCmd {
    /// `kb-code comments list --repo R [--path P] [--kind K] [--keyword K]
    /// [--state S] [--limit N] [--offset N]` — the paged index.
    List {
        #[arg(long)]
        repo: String,
        /// Path PREFIX filter (`app/models/`).
        #[arg(long)]
        path: Option<String>,
        /// doc | annotation | directive | section | licence | generated |
        /// commented_code | prose.
        #[arg(long)]
        kind: Option<String>,
        /// Exact annotation keyword (`TODO`, `FIXME`, …).
        #[arg(long)]
        keyword: Option<String>,
        /// none | fresh | drifted | unknown | aged | unreasoned.
        #[arg(long)]
        state: Option<String>,
        #[arg(long)]
        limit: Option<usize>,
    ...
    /// `kb-code comments show --repo R <PATH:LINE>` — the one block
    /// covering that line, with its state decomposition. A client-side
    /// pick over the file feed: a comment has no id of its own, and
    /// minting one would be a second identity for a range that already
    /// addresses itself.
    Show {
        /// `PATH:LINE`.
        target: String,
    ...
    /// `kb-code comments file --repo R --path P` — every block in one
    /// file, in line order.
    File {
        #[arg(long)]
        repo: String,
    ...
    /// `kb-code comments summary --repo R` — exact per-kind and
    /// per-keyword counts plus the two blame-free state lanes.
    Summary {
        #[arg(long)]
        repo: String,
        #[arg(long, default_value = "http://127.0.0.1:4747")]
    ...
    /// `kb-code comments audit --repo R [--path P] [--limit N]` — the
    /// ACTIONABLE slice in one compact list: drifted docs, aged
    /// annotations, unreasoned suppressions. Three narrow reads, never
    /// "every comment in the repo".
    Audit {
        #[arg(long)]
        repo: String,
        #[arg(long)]
    ...
    /// `kb-code comments keywords` — the annotation vocabulary actually in
    /// force (`[comments] keywords`, or the shipped default set).
    Keywords {
        #[arg(long, default_value = "http://127.0.0.1:4747")]
        daemon: String,
        #[arg(long)]
...
async fn run(cli: Cli) -> Result<()> {
    match cli.cmd {
        ...
        Cmd::Comments { cmd } => comments_cmd(cmd).await,
...
async fn review_start_pr_cmd(
    ...
    let (status, body) =
        post_json_raw(&client, daemon, "/api/reviews/pr?async=1", &payload).await?;
    if status == reqwest::StatusCode::ACCEPTED {
        ...
        return match classify_start_pr_job(&terminal) {
            ...
            StartPrJob::Failed { error, error_type } => {
                // The refusal verbatim — the stale-mirror message's retry
                // command is IN the text, so nothing is re-worded here.
    ...
    // (202 never happens) — keep the legacy handling byte-identical.
    if json {
        println!("{}", serde_json::to_string_pretty(&body)?);
    }
    if !status.is_success() {
...
// --- V72-J1 — `comments/1` ---------------------------------------------
//
// Every request this family sends is built by ONE of the four functions
// below, so `cli_requests_send_every_param_their_route_requires` can walk
// the server's `comments::V72_J1_ROUTES` declaration against what actually
// goes out (the v7.0 dead-surface defect: a verb that never sends the
// param its route requires).

/// The `GET /api/comments` request: `(path, query)`.
#[allow(clippy::too_many_arguments)]
fn comments_list_request(
    repo: &str,
    path: Option<&str>,
    kind: Option<&str>,
    keyword: Option<&str>,
    ...
    (kb_code_server::comments::routes::COMMENTS_ROUTE.path, query)
}

/// The `GET /api/comments/file` request: `(path, query)`.
fn comments_file_request(repo: &str, path: &str) -> (&'static str, Vec<(&'static str, String)>) {
    (
        kb_code_server::comments::routes::COMMENTS_FILE_ROUTE.path,
        vec![("repo", repo.to_string()), ("path", path.to_string())],
    )
}

/// The `GET /api/comments/summary` request: `(path, query)`.
fn comments_summary_request(repo: &str) -> (&'static str, Vec<(&'static str, String)>) {
    (
        kb_code_server::comments::routes::COMMENTS_SUMMARY_ROUTE.path,
        vec![("repo", repo.to_string())],
    )
}

/// The `GET /api/comments/keywords` request: `(path, query)`. Daemon-wide
/// config, so it carries no `repo`.
fn comments_keywords_request() -> (&'static str, Vec<(&'static str, String)>) {
    (
        kb_code_server::comments::routes::COMMENTS_KEYWORDS_ROUTE.path,
        Vec::new(),
    )
}

async fn comments_cmd(cmd: CommentsCmd) -> Result<()> {
    match cmd {
        CommentsCmd::List {
            repo,
            path,
            ...
            let client = http_client()?;
            let (route, query) = comments_list_request(
                &repo,
                path.as_deref(),
                kind.as_deref(),
                keyword.as_deref(),
        ...
        CommentsCmd::Show {
            ...
            let (path, line) = parse_comment_address(&target)?;
            let client = http_client()?;
            let (route, query) = comments_file_request(&repo, &path);
            let body = get_json(&client, &daemon, route, &as_query_pairs(&query)).await?;
            let empty = Vec::new();
            let hit = body["comments"]
                .as_array()
                .unwrap_or(&empty)
                .iter()
                .find(|c| {
                    let a = c["line_start"].as_i64().unwrap_or(0);
                    let b = c["line_end"].as_i64().unwrap_or(0);
                    a <= line && line <= b
                })
                .cloned();
            match hit {
                    ...
                    if json {
                        println!(
                            ...
                            serde_json::to_string_pretty(
                                &serde_json::json!({ "comment": null, "reason": "no comment block covers that line" })
                            )?
                        );
                    } else {
                        println!("(no comment block covers {path}:{line})");
                    }
                    Ok(())
                }
                Some(c) => {
                    ...
                    println!("{}", comment_headline(&c));
                    for l in c["text"].as_str().unwrap_or("").lines() {
                        println!("    {l}");
                    }
                    if c["text_truncated"].as_bool().unwrap_or(false) {
                        println!("    … (text truncated at the stored cap)");
                    }
                    Ok(())
                }
            }
        ...
        CommentsCmd::Summary { repo, daemon, json } => {
            let client = http_client()?;
            let (route, query) = comments_summary_request(&repo);
            let body = get_json(&client, &daemon, route, &as_query_pairs(&query)).await?;
            if json {
                println!("{}", serde_json::to_string_pretty(&body)?);
                return Ok(());
            }
            println!("{} comments in {repo}", body["total"].as_i64().unwrap_or(0));
            print_counts("by kind", &body["by_kind"]);
            print_counts("by keyword", &body["by_keyword"]);
            print_counts("by state", &body["by_state"]);
            let basis = &body["state_basis"];
            println!(
                "  (state counts cover {} only; {} are computed from git blame per request — \
                 ask `kb-code comments list --state drifted`)",
                join_strs(&basis["lanes"]),
                join_strs(&basis["excluded"]),
            );
            if basis["truncated"].as_bool().unwrap_or(false) {
                println!(
                    "  (scanned {} candidate rows, the bound — counts are a floor)",
                    basis["candidates_scanned"].as_u64().unwrap_or(0)
                );
            }
            Ok(())
        }
        CommentsCmd::Audit {
            ...
            // The lane list is the SERVER's own `ACTIONABLE_STATES`, not a
            // restatement of it — a state added there shows up here with no
            // edit, and one removed cannot linger as a dead query.
            for state in kb_code_server::comments::drift::ACTIONABLE_STATES {
                let state = *state;
                let (route, query) = comments_list_request(
                    &repo,
                    path.as_deref(),
                ...
                let body = get_json(&client, &daemon, route, &as_query_pairs(&query)).await?;
                lanes.push((state, body));
            }
            if json {
                let obj: serde_json::Map<String, serde_json::Value> =
                    lanes.into_iter().map(|(s, b)| (s.to_string(), b)).collect();
                println!(
                    "{}",
            ...
            let mut any = false;
            for (state, body) in &lanes {
                let empty = Vec::new();
                let rows = body["comments"].as_array().unwrap_or(&empty);
                if rows.is_empty() {
                    continue;
                }
                any = true;
                println!("{state} ({}):", body["total"].as_u64().unwrap_or(0));
                for c in rows {
                    println!("  {}", audit_line(c));
                }
                print_comment_basis(body);
            }
            if !any {
        ...
        CommentsCmd::Keywords { daemon, json } => {
            let client = http_client()?;
            let (route, query) = comments_keywords_request();
            let body = get_json(&client, &daemon, route, &as_query_pairs(&query)).await?;
            if json {
                println!("{}", serde_json::to_string_pretty(&body)?);
                return Ok(());
            ...
            println!(
                "keywords ({}): {}",
                body["source"].as_str().unwrap_or("?"),
                join_strs(&body["keywords"])
            );
            println!("rubocop defaults: {}", join_strs(&body["rubocop_defaults"]));
            println!("legacy extra:     {}", join_strs(&body["legacy_extra"]));
            println!(
                "todo family:      {}  (what `kb-code todos` reports)",
                join_strs(&body["todo_family"])
            );
            Ok(())
        }
    }
...
/// `PATH:LINE`, the address `kb-code comments show` takes. Rides the
/// EXISTING `parse_path_line` (`kb-code why`'s own target grammar) rather
/// than a second one, and REFUSES the line-less form: a comment `show`
/// with no line has nothing to show.
fn parse_comment_address(s: &str) -> Result<(String, i64)> {
    let (path, line) = parse_path_line(s);
    let line = line.ok_or_else(|| anyhow::anyhow!("expected PATH:LINE, got {s:?}"))?;
    if path.is_empty() {
        anyhow::bail!("expected PATH:LINE, got {s:?}");
    }
    Ok((path, i64::from(line)))
}
...
fn print_counts(label: &str, v: &serde_json::Value) {
...
/// `path:line-line  kind[/keyword]  state  first line of text`.
fn comment_headline(c: &serde_json::Value) -> String {
    let path = c["path"].as_str().unwrap_or("?");
    let a = c["line_start"].as_i64().unwrap_or(0);
    let b = c["line_end"].as_i64().unwrap_or(0);
    let kind = c["kind"].as_str().unwrap_or("?");
    let label = match c["keyword"].as_str() {
        Some(k) => format!("{kind}/{k}"),
        None => kind.to_string(),
    };
    let state = c["state"]["state"].as_str().unwrap_or("none");
    let range = if a == b {
        format!("{a}")
    } else {
        format!("{a}-{b}")
    ...
    let first = c["text"]
        ...
        .next()
        .unwrap_or("");
    format!("{path}:{range}\t{label}\t{state}\t{first}")
}

/// One agent-actionable line: the address, why it is here, and the
/// arithmetic behind that — never a verdict.
fn audit_line(c: &serde_json::Value) -> String {
    let path = c["path"].as_str().unwrap_or("?");
    let a = c["line_start"].as_i64().unwrap_or(0);
    let st = &c["state"];
    let detail = match st["state"].as_str().unwrap_or("") {
        "drifted" => format!(
            "{} documents {} (lines {}-{}); the body changed {}d after the doc ({} vs {})",
            c["kind"].as_str().unwrap_or("doc"),
            c["symbol"]["name"].as_str().unwrap_or("?"),
            c["symbol"]["line_start"].as_i64().unwrap_or(0),
            c["symbol"]["line_end"].as_i64().unwrap_or(0),
            st["age_days"].as_i64().unwrap_or(0),
            short_sha(st["code_commit"].as_str().unwrap_or("")),
            short_sha(st["doc_commit"].as_str().unwrap_or("")),
        ),
        "aged" => format!(
            "{} due {} ({}d past): {}",
            c["keyword"].as_str().unwrap_or("TODO"),
            st["on_date"].as_str().unwrap_or("?"),
            st["age_days"].as_i64().unwrap_or(0),
            c["keyword_text"].as_str().unwrap_or(""),
        ),
        "unreasoned" => format!(
            "{} suppression with no reason: {}",
            st["tool"].as_str().unwrap_or("?"),
            c["text"]
                .as_str()
                .unwrap_or("")
                .lines()
...
fn print_comment_rows(body: &serde_json::Value) {
    let empty = Vec::new();
    let rows = body["comments"].as_array().unwrap_or(&empty);
    if rows.is_empty() {
        println!("(no comments matched)");
        return;
    }
    for c in rows {
        println!("{}", comment_headline(c));
...
/// Print what the response says about its own bounds — the honest half of
/// every read this family makes.
fn print_comment_basis(body: &serde_json::Value) {
    let total = body["total"].as_u64().unwrap_or(0);
    let shown = body["comments"].as_array().map(Vec::len).unwrap_or(0);
    if body["truncated"].as_bool().unwrap_or(false) {
        println!("({shown} of {total} shown — pass --offset to page)");
    }
    for n in body["notes"].as_array().into_iter().flatten() {
        if let Some(s) = n.as_str() {
            println!("note: {s}");
...
mod tests {
    ...
    fn cli_requests_send_every_param_their_route_requires() {
        let built = [
            ...
            // V72-J1 — the `comments/1` family joins the SAME walk. A
            // route declared in `comments::V72_J1_ROUTES` that no verb
            // builds a request for fails HERE, by path.
            comments_list_request("repo", None, None, None, None, None, None),
    ...
    // --- V72-J1: `comments list`'s own declaration↔consumer walk ----------

    #[test]
    fn every_comments_list_flag_reaches_the_wire_and_nothing_else_does() {
        use clap::CommandFactory;
        let cli = Cli::command();
        let comments = cli
            .get_subcommands()
            .find(|c| c.get_name() == "comments")
            .expect("`kb-code comments` exists");
        let list = comments
            .get_subcommands()
            .find(|c| c.get_name() == "list")
            .expect("`kb-code comments list` exists");
        let declared: Vec<String> = list
            .get_arguments()
            .filter_map(|a| a.get_long().map(str::to_string))
            .collect();
        // Transport + output format, not wire params.
        const LOCAL: &[&str] = &["daemon", "json", "help"];

        let (_, query) = comments_list_request(
            "r",
            Some("app/"),
            Some("doc"),
            Some("TODO"),
        ...
        let sent: Vec<&str> = query.iter().map(|(k, _)| *k).collect();
        for flag in &declared {
            if LOCAL.contains(&flag.as_str()) {
                continue;
            }
            assert!(
                sent.contains(&flag.as_str()),
                "`kb-code comments list --{flag}` is declared but never reaches the wire"
            );
        }
        for k in &sent {
            assert!(
                declared.iter().any(|d| d == k),
                "the request sends {k:?}, which no `comments list` flag declares"
            );
        }
    }

    #[test]
    fn the_audit_verb_asks_for_exactly_the_servers_actionable_states() {
        // A source scan, the same trade `git_argv_lint` makes: it proves the
        // expression occurs, not that it is reached. What it DOES stop is a
        // second, hand-written copy of the lane list drifting from the one
        // `drift::ACTIONABLE_STATES` declares.
        const SRC: &str = include_str!("main.rs");
        assert!(
            SRC.contains("kb_code_server::comments::drift::ACTIONABLE_STATES"),
            "`comments audit` must read the server's own lane list"
        );
        for s in kb_code_server::comments::drift::ACTIONABLE_STATES {
            assert!(
                kb_code_server::comments::drift::STATE_NAMES.contains(s),
                "{s} is actionable but not a declared state"
            );
        }
    }
