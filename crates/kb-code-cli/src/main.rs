...
async fn run(cli: Cli) -> Result<()> {
    match cli.cmd {
        ...
        Cmd::Search {
            ...
            None => match query {
                Some(q) => search_unified_cmd(&daemon, repo.as_deref(), &q, limit, json).await,
                None => anyhow::bail!(
...
async fn search_unified_cmd(
    ...
    let body = get_json(&client, daemon, "/api/search", &query).await?;
    if json {
        println!("{}", serde_json::to_string_pretty(&body)?);
        return Ok(());
    }
...
async fn usages_cmd(
    ...
    let body = get_json(&client, daemon, "/api/usages", &q).await?;
    if json {
        println!("{}", serde_json::to_string_pretty(&body)?);
        return Ok(());
    }
...
async fn bench_search_cmd(
    ...
    for q in &queries {
        ...
        let start = std::time::Instant::now();
        let body = get_json(&client, daemon, "/api/search", &query_params).await?;
