...
impl WatchItem {
    ...
    /// V71-X1 — the item's own recency timestamp, for [`seed_seen`]'s
    /// `since` window. `VerdictItem` names its field `at`, not
    /// `updated_at`; both are unix seconds.
    pub fn updated_at(&self) -> i64 {
        match self {
            WatchItem::Annotation(a) => a.updated_at,
            WatchItem::Verdict(v) => v.at,
            WatchItem::Finding(f) => f.updated_at,
        }
    }
}
...
/// (so the caller can surface the initial open set once). V71-X1's
/// `since` is an ADDITIVE surfacing rule on top of `backlog`, never a
/// replacement for it: an item whose own [`WatchItem::updated_at`] is at
/// or after `since` surfaces too, even when `backlog` is false — `kb-code
/// watch`'s "catch me up since I last watched" window. `since: None`
/// (every pre-V71-X1 caller) is byte-identical to the old two-argument
/// behavior.
pub fn seed_seen(
    ...
    if backlog {
        ...
        for item in items {
            ...
            if since.is_some_and(|s| item.updated_at() >= s) {
                surfaced.push(item.clone());
            }
        }
        surfaced
    }
...
/// `lane_tag`, when `Some`, is V71-X1's `kb-code watch <lane>...` fan-out
/// marker: spliced into the JSON object as `"lane"` (never nested — one
/// flat envelope, `"kind"` still names the item's OWN shape) and prefixed
/// on the human line as `[tag] `. `None` for every pre-V71-X1 caller
/// (`kb-code annotate watch` on its own) — byte-identical output.
/// Splice `"lane": tag` into `v` (a [`format_json`] object) — never a
/// nested envelope, so `"kind"` and every other item field stay at the
/// SAME level. A no-op when `tag` is `None` or `v` isn't an object (never
/// true for a [`format_json`] output, but this stays total rather than
/// panicking on a future shape change).
...
pub fn emit(item: &WatchItem, json: bool, lane_tag: Option<&str>) {
    if json {
        println!("{}", apply_lane_tag(format_json(item), lane_tag));
    } else {
        match lane_tag {
            Some(tag) => println!("[{tag}] {}", format_human(item)),
            None => println!("{}", format_human(item)),
        }
    }
...
pub struct WatchArgs {
    ...
    /// V71-X1 — surface an item whose OWN `updated_at`/`at` is at or after
    /// this unix timestamp on the FIRST seed, same as `backlog` but
    /// bounded to a window rather than "everything open." `None` (the
    /// pre-V71-X1 default) never surfaces anything on the seed unless
    /// `backlog` says so.
    pub since: Option<i64>,
    /// V71-X1 — see [`emit`]'s doc. `None` for the standalone `annotate
    /// watch` command.
...
pub async fn run(args: WatchArgs) -> Result<()> {
    ...
    let mut surfaced = seed_seen(&mut seen, &initial, args.backlog, args.since);
    surfaced = filter_ignored(&surfaced, &args.ignore_author);
    for item in &surfaced {
        emit(item, args.json, args.lane_tag.as_deref());
    }
...
async fn surface_verdict(
    ...
    let fresh = filter_ignored(&fresh, &args.ignore_author);
    for it in &fresh {
        emit(it, args.json, args.lane_tag.as_deref());
        reset_idle(deadline, args.timeout_secs);
    }
    Ok(args.once && !fresh.is_empty())
}
...
mod tests {
    ...
    fn seed_without_backlog_does_not_surface() {
        ...
        let surfaced = seed_seen(&mut seen, &items, false, None);
    ...
    fn seed_with_backlog_surfaces_once() {
        let mut seen = HashSet::new();
        let items = vec![ann("a1", 1, false, false)];
        let first = seed_seen(&mut seen, &items, true, None);
        assert_eq!(first.len(), 1);
        assert_eq!(first[0].seen_key(), items[0].seen_key());
        assert!(take_new(&mut seen, &items).is_empty());
    }

    // V71-X1 — `since` surfacing, independent of `backlog`.
    #[test]
    fn seed_with_since_surfaces_only_items_at_or_after_the_window() {
        let mut seen = HashSet::new();
        let items = vec![ann("old", 100, false, false), ann("new", 200, false, false)];
        let surfaced = seed_seen(&mut seen, &items, false, Some(200));
        assert_eq!(
            surfaced.iter().map(|i| i.seen_key()).collect::<Vec<_>>(),
            vec![items[1].seen_key()],
            "only the item at/after `since` surfaces"
        );
        // Both are seeded regardless — a later identical refetch is quiet.
        assert_eq!(seen.len(), 2);
        assert!(take_new(&mut seen, &items).is_empty());
    }

    #[test]
    fn seed_with_since_and_backlog_defers_to_backlog_surfacing_everything() {
        let mut seen = HashSet::new();
        let items = vec![ann("old", 100, false, false), ann("new", 200, false, false)];
        let surfaced = seed_seen(&mut seen, &items, true, Some(200));
        assert_eq!(
            surfaced.len(),
            2,
            "backlog surfaces everything regardless of since"
        );
    }

    #[test]
    fn seed_with_none_since_never_surfaces_without_backlog() {
        let mut seen = HashSet::new();
        let items = vec![ann("a1", 999, false, false)];
        let surfaced = seed_seen(&mut seen, &items, false, None);
        assert!(surfaced.is_empty(), "the pre-V71-X1 default stays silent");
    }

    #[test]
    fn apply_lane_tag_splices_flat_never_nested() {
        let v = format_json(&ann("a1", 1, false, false));
        let tagged = apply_lane_tag(v.clone(), Some("annotate"));
        assert_eq!(tagged["lane"], "annotate");
        assert_eq!(tagged["kind"], v["kind"], "the item's own fields survive");
        assert_eq!(tagged["id"], v["id"]);
    }

    #[test]
    fn apply_lane_tag_none_is_a_byte_identical_no_op() {
        let v = format_json(&ann("a1", 1, false, false));
        assert_eq!(apply_lane_tag(v.clone(), None), v);
    }
    ...
    fn replay_refetch_is_idempotent() {
        ...
        let _ = seed_seen(&mut seen, &items, false, None);
        assert!(take_new(&mut seen, &items).is_empty());
    }
    ...
    fn disposition_change_surfaces_once_and_author_is_the_disposer() {
        ...
        let _ = seed_seen(&mut seen, std::slice::from_ref(&before), false, None);
    ...
    fn unchanged_publish_state_does_not_resurface_but_a_real_change_does() {
        ...
        let _ = seed_seen(&mut seen, std::slice::from_ref(&f), false, None);
