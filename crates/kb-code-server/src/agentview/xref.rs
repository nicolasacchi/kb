...
pub(crate) fn resolve_defs(
    ...
    let fuzzy = symbol_index.search(store, repos, symbol, limit, &search::LaneOpts::default())?;
...
pub(crate) fn resolve_refs(
    ...
    let resp = search::search_text(
        ...
        // V71-D1 — the agent lanes take the DEFAULTS, not the operator's
        // `[search]` flags: these are exact-symbol reference reads whose
        // consumers class each hit themselves, not a ranked human surface.
