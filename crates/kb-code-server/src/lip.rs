...
pub(crate) async fn overlay_usages(
    ...
    for loc in parse_locations(&answer.results) {
        let key = (loc.path.clone(), loc.line, loc.col);
        if !seen.insert(key) {
            continue;
        }
        new_total += 1;
        if out.exact.len() < limit {
            let context = lip_context_for(repo, &loc.path, loc.line);
...
/// never an error.
fn lip_context_for(repo: &crate::config::RepoEntry, path: &str, line: u32) -> String {
    let Ok(read) = read_repo_file(repo, path, None) else {
        return String::new();
    };
