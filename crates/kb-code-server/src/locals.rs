...
pub fn bind_locals(lang_id: &str, source: &[u8]) -> Result<Vec<Binding>> {
    ...
    if !scope_ranges.iter().any(|&(s, e)| s == 0 && e >= root_end) {
        scope_ranges.push((0, root_end.max(1)));
    }
...
fn build_scope_tree(ranges: &[(usize, usize)]) -> Vec<Scope> {
    // Dedup identical ranges, sort by start ASC then end DESC so a parent
    // (wider) appears before its children when starts equal.
    let mut uniq: Vec<(usize, usize)> = ranges.to_vec();
