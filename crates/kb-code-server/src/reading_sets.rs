...
/// V71-G0 (kbc-seq/1, design §P6) — a tour and a trail are `reading_sets`
/// rows too. Widening this list rather than adding two tables is the same
/// ruling invariant 9 already records for `workspace`: "the familiar names
/// … kept as documented aliases" over "a resolver over the three existing
/// tables". Purely ADDITIVE — `GET /api/sets` still defaults to `'set'`,
/// so no existing caller sees a row it did not see before.
pub const SET_KIND_TOUR: &str = "tour";
pub const SET_KIND_TRAIL: &str = "trail";
pub const SET_KINDS: [&str; 4] = [
    SET_KIND_SET,
    SET_KIND_WORKSPACE,
    SET_KIND_TOUR,
    SET_KIND_TRAIL,
...
pub struct SetSummary {
    ...
    /// V71-G0 (D26) — the workspace this projection is bound to, when it
    /// is bound to one. Omitted when absent (same convention as
    /// `ref_label` beside it).
...
pub struct SetView {
    ...
    /// Written by `PATCH /api/sets/{id}`'s own `workspace_id` field, read
    /// back here so a binding is never a write with no visible effect.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub workspace_id: Option<String>,
}
...
pub struct PatchSetBody {
    ...
    #[serde(default)]
    pub description_md: Option<String>,
    /// V71-G0 (D26) — bind this projection to a workspace, or UNBIND it
    /// with an empty/whitespace-only string. Unlike every `COALESCE`-
    /// updated field above, a present-but-empty value here means CLEAR,
    /// not "leave untouched": "convert to and from it with one key" needs
    /// the *from* direction to be expressible (see
    /// `store::Store::set_reading_set_workspace`). The target is validated
    /// to be an existing `'workspace'` row in the SAME repo, and never
    /// this set itself.
    #[serde(default)]
    pub workspace_id: Option<String>,
}
...
pub async fn patch_set(
    ...
    // fields already have.
    let ref_label = validate_ref_label(body.ref_label.as_deref())?;
    // `Option<Option<String>>`: outer `None` = the key was absent (leave
    // the binding alone), inner `None` = an explicit clear. The one field
    // on this route that can express "unset" — see the field's own doc.
    let workspace_update: Option<Option<String>> = body.workspace_id.as_ref().map(|w| {
        let t = w.trim();
    ...
    let (view, repo_name) = state
        ...
        .run_blocking(move |store| {
            ...
            // V71-G0 — the workspace binding, validated against the store
            // BEFORE it is written: a dangling `workspace_id`, one
            // pointing into another repo, one naming a plain set, or a
            // self-binding are all 400s with nothing written.
            if let Some(target) = &workspace_update {
                if let Some(w) = target {
                    if w == &id {
                        return Err(ApiError::bad_request(
                            "workspace_id must not be this set itself",
                        ));
                    }
                    let ws = store.get_reading_set(w)?.ok_or_else(|| {
                        ApiError::bad_request(format!("no such workspace: {w:?}"))
                    })?;
                    if ws.repo_id != existing.repo_id {
                        return Err(ApiError::bad_request(format!(
                            "workspace {w:?} belongs to a different repo"
                        )));
                    }
                    if ws.kind != SET_KIND_WORKSPACE {
                        return Err(ApiError::bad_request(format!(
                            "{w:?} is a {:?}, not a workspace",
                            ws.kind
                        )));
                    }
                }
                store.set_reading_set_workspace(&id, target.as_deref(), now)?;
            }
