...
pub struct FileHit {
    ...
    pub score: f64,
    /// V71-D1 — the STRUCTURAL ordering tier this hit sits in
    /// (`matcher::MatchTier`): every `exact` sorts above every `prefix`,
    /// which sorts above every `fuzzy`, and `score` only ever orders WITHIN
    /// one tier. `None` on a `recent` hit — there is no needle to be exact
    /// about.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tier: Option<MatchTier>,
    /// V71-D1 — UTF-16 `[start, end)` offsets of the matched characters in
    /// `path`, for the client's highlight pass (`matcher`'s module doc).
    /// Empty on a `recent` hit.
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub ranges: Vec<[u32; 2]>,
    /// V71-D1 — the stable hit id (`search::hit_id`), so an agent can name
    /// this row in a later turn.
    pub hit_id: String,
    /// V71-D1 — the honest ranking decomposition, present iff the caller
    /// asked (`explain:1`). See [`super::Explain`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub explain: Option<super::Explain>,
    /// V71-D2 — the blob this hit was indexed from, the per-hit half of the
    /// LLM contract's staleness (`unified::Freshness` is the other). An
    /// agent that quotes a line can re-check the blob later and be told
    /// honestly that the file changed, instead of silently reading a
    /// different one.
    ///
    /// `None` on a `recent` (empty-query) hit: that row comes out of the
    /// open history, which records WHERE a file was opened, not what it
    /// contained — inventing the current blob there would answer a question
    /// the row never asked.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub blob_sha: Option<String>,
}

/// One entry of [`FileIndex`]'s per-repo snapshot. V71-D2 widened it from a
/// bare `String` path so a hit can carry the blob it was indexed from — the
/// blob comes out of the SAME `list_files` read the paths already came from,
/// so this costs one extra `String` per file and no extra query.
...
pub struct FileIndex {
    cache: Mutex<HashMap<i64, GenCached<Vec<FileEntry>>>>,
...
impl FileIndex {
    ...
    fn snapshot(&self, store: &Store, repo_id: i64) -> Result<Arc<Vec<FileEntry>>, StoreError> {
        ...
        let paths: Arc<Vec<FileEntry>> = Arc::new(
            rows.into_iter()
                .map(|r| FileEntry {
                    path: r.path,
    ...
    pub fn search(
        ...
        // V71-D1 — ONE matcher (`search::matcher`), which also hands back
        // the hard tier and the UTF-16 match indices; this lane no longer
        // drives nucleo itself.
        let mut matcher = NameMatcher::new(query, HaystackKind::Path);
        ...
        for (repo_name, repo_id) in repos {
            ...
            for entry in paths.iter() {
                ...
                let Some(m) = matcher.score(path, HaystackKind::Path) else {
                    continue;
                };
                let base = m.score as f64;
                let mut score = base;
                let mut explain = opts
                    .explain
                    .then(|| super::Explain::new("files", m.tier, base));
                // Every factor: skipped ENTIRELY when its flag is off (not
                // multiplied by a neutral 1.0), and surfaced on the
                // decomposition iff it is on — kb's MI-W5.R discipline, see
                // `super::Factors`.
                if opts.factors.frecency {
                    let boost = recency
                        .get(path)
                    ...
                    if let Some(e) = explain.as_mut() {
                        e.delta("frecency", boost, "open-history recency, 3-day half-life");
                    }
                }
                ...
                hits.push(FileHit {
                    ...
                    tier: Some(m.tier),
                    ranges: m.ranges,
                    hit_id: super::hit_id("files", repo_name, path, ""),
                    explain,
                    blob_sha: Some(entry.blob_hash.clone()),
                });
            }
        }

        // The tier is a HARD key ABOVE the score: no learned or provenance
        // factor can lift a fuzzy hit over an exact one (`matcher`'s module
        // doc — the JetBrains Search-Everywhere failure mode, designed out).
        hits.sort_by(|a, b| {
            a.tier
                .cmp(&b.tier)
                .then_with(|| {
    ...
    pub fn recent(
        ...
        let rows = store.recent_file_opens(&repo_ids, limit, opts.path_filter)?;
        Ok(rows
            .into_iter()
            .map(|(repo_id, path, opened_at)| {
                let repo_name = repos
                    ...
                    .unwrap_or_default();
                let hit_id = super::hit_id("files", &repo_name, &path, "");
                FileHit {
                    repo: repo_name,
                    path,
                    score: opened_at as f64,
                    // No needle, so no tier and no match indices — an empty
                    // ranges list is the honest answer, not a fabricated
                    // whole-path highlight.
                    tier: None,
                    ranges: Vec::new(),
                    hit_id,
                    explain: None,
                    // See the field doc: a recents row has no blob claim.
                    blob_sha: None,
                }
            })
            .collect())
...
mod tests {
    ...
    fn empty_query_recent_returns_most_recently_opened_first() {
        ...
        let index = FileIndex::new();
        let hits = index
            .recent(&store, &[("r".into(), repo_id)], 10, &LaneOpts::default())
            .unwrap();
        let paths: Vec<&str> = hits.iter().map(|h| h.path.as_str()).collect();
    ...
    fn recent_path_filter_narrows_the_recency_window_not_just_the_page() {
        ...
        let index = FileIndex::new();
        let unfiltered = index
            .recent(&store, &[("r".into(), repo_id)], 2, &LaneOpts::default())
            .unwrap();
        assert!(!unfiltered.iter().any(|h| h.path == "keep/old.rs"));

        let filtered = index
            .recent(&store, &[("r".into(), repo_id)], 2, &opts_path("keep/"))
            .unwrap();
        let paths: Vec<&str> = filtered.iter().map(|h| h.path.as_str()).collect();
        assert_eq!(paths, vec!["keep/old.rs"], "got {paths:?}");
    }

    /// V71-D2 — the LLM contract's per-hit staleness. A SEARCH hit carries
    /// the blob it was indexed from; a RECENTS hit does not, and the
    /// difference is deliberate (see `FileHit::blob_sha`'s doc) rather than
    /// an oversight to be "fixed" by filling it in from the current index.
    #[test]
    fn a_search_hit_carries_its_indexed_blob_and_a_recents_hit_does_not() {
        let (_tmp, store) = open_store();
        let repo_id = store.upsert_repo("r", "/tmp/r").unwrap();
        seed_files(&store, repo_id, &["src/lib.rs"]);
        ...
        let hits = index
            ...
            .unwrap();
        assert_eq!(hits[0].blob_sha.as_deref(), Some("hash0"));

        let recents = index
            .recent(&store, &[("r".into(), repo_id)], 10, &LaneOpts::default())
            .unwrap();
        assert_eq!(recents[0].path, "src/lib.rs");
        assert_eq!(recents[0].blob_sha, None);
    }
}
