...
use super::grammar::{self, Filters, Lane, TextMode, LANE_ORDER};
use super::sessions;
use super::MAX_LIMIT;
...
fn matches_filters(path: &str, filters: &Filters) -> bool {
    if let Some(want_path) = &filters.path {
        if !path.to_lowercase().contains(&want_path.to_lowercase()) {
            return false;
        }
    }
...
pub async fn run(
    ...
    if raw_q.trim().is_empty() {
        let files_section = match routes::resolve_search_repos(state, repo_param) {
            Ok(repos) => {
                ...
                let result = state
                    .store
                    .run_blocking(move |store| file_index.recent(store, &repos, limit, None))
                    .await;
                match result {
                    Ok(hits) => {
    ...
    let (files_out, symbols_out, text_out, semantic_out, sessions_out, transcripts_out) = tokio::join!(
        async {
            ...
            Some(run_files(state, &repos_result, query, filters, limit).await)
        },
        async {
            if !want(Lane::Symbols) {
                return None;
            }
            Some(run_symbols(state, &repos_result, query, filters, limit).await)
        },
        async {
            if !want(Lane::Text) {
                return None;
            }
            Some(run_text(state, &repos_result, query, filters, text_mode, limit).await)
        },
        async {
    ...
    let result = state
        ...
        .run_blocking(move |store| {
            ...
            let hits = if q.is_empty() {
                file_index.recent(store, &repos, raw_limit, path_filter)
            } else {
                let now_ms = chrono::Utc::now().timestamp_millis();
                file_index.search(store, &repos, &q, raw_limit, now_ms, path_filter)
            };
            hits.map(|hits| {
        ...
        .await;
    match result {
        Ok((filtered, raw_len)) => section(Lane::Files, to_value(&filtered), raw_len >= raw_limit),
        Err(e) => unavailable(Lane::Files, e.to_string()),
    }
}
...
async fn run_symbols(
    ...
    let result = state
        ...
        .run_blocking(move |store| {
            ...
            let path_filter = filters.path.as_deref();
            symbol_index
                .search(store, &repos, &q, raw_limit, path_filter)
                .map(|hits| {
                    let raw_len = hits.len();
                    let mut filtered: Vec<_> = hits
                        .into_iter()
    ...
    match result {
        Ok((filtered, raw_len)) => {
            section(Lane::Symbols, to_value(&filtered), raw_len >= raw_limit)
        }
