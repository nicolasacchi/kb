...
pub(crate) fn usages_at(
    ...
    let mut exact: Vec<UsageRow> = Vec::new();
    let mut likely: Vec<UsageRow> = Vec::new();
    let mut candidate: Vec<UsageRow> = Vec::new();
    ...
    for (p, occ) in &refs {
        ...
        let row = make_row(p, occ, &text, "ref", acc);
        match class {
            CLASS_LIKELY => likely.push(row),
            CLASS_EXACT => exact.push(row),
            _ => candidate.push(row),
...
fn push_usage(
    ...
    out.push(make_row(path, occ, content, kind, acc));
}
