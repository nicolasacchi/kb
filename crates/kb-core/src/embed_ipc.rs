...
fn apply_nice_before_run(cmd: &mut Command, nice: i32) {
    ...
    nice_unix::install(cmd, nice, embedder_cpu_cap());
}

#[cfg(not(unix))]
fn apply_nice_before_run(_cmd: &mut Command, _nice: i32) {}

/// PF-F1 — CPU-affinity cap for the kb-embedder subprocess. fastembed
/// hardcodes ORT's intra-op thread count to `available_parallelism()` at
/// init, which honours `sched_getaffinity` — so constraining the
/// subprocess's affinity BEFORE exec is the one non-patch lever over its
/// thread pool. Measured on the 8-core reference box (batch=1 / batch=32
/// emb/s): all-8 = 51.6/66.0 · 4 cores = 62.7/93.9 (+22%/+42%) · 2 =
/// 56.2/76.4 · 1 = 49.8/51.1 — a small model over-parallelized across
/// every core loses to thread-sync overhead, and the historical
/// "batching doesn't help" finding was an artifact of that thrash.
/// Default: min(4, available cores). `KB_EMBEDDER_CPUS` overrides
/// (0 = no cap, restoring the old all-cores behaviour).
#[cfg(unix)]
fn embedder_cpu_cap() -> usize {
    if let Ok(v) = std::env::var("KB_EMBEDDER_CPUS") {
...
mod nice_unix {
    ...
    pub fn install(cmd: &mut Command, nice: i32, max_cpus: usize) {
        ...
        unsafe {
            ...
            (*raw).pre_exec(move || {
                ...
                // Affinity cap (see `embedder_cpu_cap`): best-effort — a
                // host where pinning fails (exotic cgroup masks) must
                // still get a working embedder, so the error is ignored,
                // never propagated.
                if max_cpus > 0 {
                    let mut set: libc::cpu_set_t = std::mem::zeroed();
                    libc::CPU_ZERO(&mut set);
                    for c in 0..max_cpus.min(libc::CPU_SETSIZE as usize) {
