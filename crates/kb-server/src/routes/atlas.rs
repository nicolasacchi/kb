...
/// PF-F1 — pure hit/miss predicate for [`AtlasPointsCache`]'s two-key
/// discipline: a cached entry is usable only when BOTH the row-set
/// generation AND the atlas-frame stamp match the caller's freshly-read
/// values. Split out (mirrors `build_atlas_points_response`'s extraction
/// above) so the two-key rule is unit-testable without a storage actor.
fn atlas_cache_is_fresh(cached: (u64, Option<i64>), current: (u64, Option<i64>)) -> bool {
    cached == current
}

/// M-a — the FULL-corpus atlas doc scan (`list_docs_with_atlas(u32::MAX)`)
/// for `ctx`, served from the per-(kb, index-generation, atlas-frame) memo
/// when warm (invariant #15). Copies `link_pairs_cached` above's std-Mutex
/// discipline VERBATIM: the guard is dropped before every `.await` below.
/// Read-only: this never calls `bump_generation`.
///
/// PF-F1 — a hit requires the stored `generation` to equal the caller's
/// `generation` **and** the stored `atlas_frame_id` to equal the newest row
/// in the `atlas_snapshots` time-lapse table. `generation` alone is NOT
/// sufficient: `update_atlas` deliberately never bumps it (see
/// [`AtlasPointsCache`]'s doc comment for why), so without this second key a
/// recompute/recluster would be invisible to every consumer of this memo.
/// `pub(crate)` — `routes::atlas_field`'s `disagreement` route reuses this
/// SAME memo so the operator-field overlay scores against the exact
/// coordinates the map itself is drawing; PF-F1 adds a third consumer,
/// `routes::docs::list`'s `?projection=atlas` row set — "one memo, many
/// consumers" rather than a second full-corpus scan. Self-contained: reads
/// its own `atlas_frame_id` (one small indexed `SELECT … LIMIT 1`, hit or
/// miss). `points()` below ALSO needs that same stamp for its own ETag and
/// reads it first, so it calls [`atlas_docs_cached_with_frame`] directly
/// with the value already in hand rather than paying for a second read
/// through this wrapper.
pub(crate) async fn atlas_docs_cached(
    ctx: &KbContext,
    generation: u64,
) -> Result<Arc<Vec<DocSummary>>, kb_core::Error> {
    let atlas_frame_id = newest_frame_id(&ctx.storage).await;
    atlas_docs_cached_with_frame(ctx, generation, atlas_frame_id).await
}

/// The body of [`atlas_docs_cached`], taking an already-read `atlas_frame_id`
/// instead of fetching it — see that fn's doc comment for the split's
/// purpose (`points()` reuses its own ETag read instead of a redundant
/// second `newest_frame_id` call).
async fn atlas_docs_cached_with_frame(
    ctx: &KbContext,
    generation: u64,
    atlas_frame_id: Option<i64>,
) -> Result<Arc<Vec<DocSummary>>, kb_core::Error> {
    {
        let guard = ctx
            .atlas_points_cache
            ...
            .unwrap_or_else(|e| e.into_inner());
        if let Some(c) = guard.as_ref() {
            if atlas_cache_is_fresh(
                (c.generation, c.atlas_frame_id),
                (generation, atlas_frame_id),
            ) {
...
/// flag on every point — **plus** (PF-F1) the newest atlas-frame id, since
/// `update_atlas` ALSO never bumps `index_generation` (see
/// [`AtlasPointsCache`]'s doc comment); without this fold a client polling
/// with `If-None-Match` would keep getting 304'd the pre-recompute
/// coordinates even after the server-side memo (`atlas_docs_cached`) has
/// correctly picked up the new layout.
pub async fn points(
    State(state): State<Arc<KbHandles>>,
    Path(kb): Path<String>,
    headers: HeaderMap,
    ...
    // PF-F1 — same fold as the pinned-set hash above, same reason: the
    // ETag must be stale-blind to nothing `atlas_docs_cached` itself isn't
    // stale-blind to. Read unconditionally (cheap, indexed `LIMIT 1`), ONCE
    // — passed straight into `atlas_docs_cached_with_frame` below instead of
    // going through the self-fetching `atlas_docs_cached` wrapper (used by
    // `routes::atlas_field`'s `disagreement` route + `routes::docs::list`'s
    // `?projection=atlas`, neither of which needs the value for anything
    // else), so this route pays for the read exactly once.
    let atlas_frame_id = newest_frame_id(&ctx.storage).await;
    let frame_tag = atlas_frame_id
        .map(|id| id.to_string())
    ...
    let etag = match &pinned {
        Some(set) => {
            ...
            format!(
                "\"atlas-points-{}-{generation}-a{frame_tag}-p{:016x}\"",
                *ETAG_BOOT_SALT,
                hasher.finish()
            )
        }
        None => format!(
            "\"atlas-points-{}-{generation}-a{frame_tag}\"",
            *ETAG_BOOT_SALT
        ),
    };
    // Conditional GET: unchanged corpus, unchanged atlas layout, AND
    // unchanged pin set ⇒ 304, no body, no scan.
    if headers
        .get(header::IF_NONE_MATCH)
        .and_then(|v| v.to_str().ok())
        .map(|v| v == etag)
    ...
    // PF-F1 — reuse the `atlas_frame_id` already read above for the ETag
    // (`atlas_docs_cached_with_frame`) instead of `atlas_docs_cached`'s
    // self-fetching wrapper, so this route pays for the frame-freshness
    // read exactly once, not twice.
    let docs = match atlas_docs_cached_with_frame(ctx, generation, atlas_frame_id).await {
        Ok(d) => d,
...
mod tests {
    ...
    // --- PF-F1: atlas_cache_is_fresh — pure two-key hit/miss predicate -----

    #[test]
    fn atlas_cache_is_fresh_requires_both_generation_and_frame_id_to_match() {
        assert!(
            atlas_cache_is_fresh((5, Some(3)), (5, Some(3))),
            "identical generation + frame id must hit"
        );
        assert!(
            !atlas_cache_is_fresh((5, Some(3)), (5, Some(4))),
            "a new atlas frame (recompute/recluster) must miss even at the \
             same generation — this is the PF-F1 stale-coords fix"
        );
        assert!(
            !atlas_cache_is_fresh((5, Some(3)), (6, Some(3))),
            "a doc write (generation bump) must miss even at the same frame"
        );
        assert!(
            atlas_cache_is_fresh((0, None), (0, None)),
            "no atlas recompute has ever run — a stable, valid fresh state"
        );
        assert!(
            !atlas_cache_is_fresh((0, None), (0, Some(1))),
            "a kb's FIRST recompute (None -> Some) must miss"
        );
    }
