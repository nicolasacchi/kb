...
//! (`atlas_docs_cached`, keyed on the storage actor's index generation AND
//! the newest atlas-frame id, PF-F1 — a recompute never bumps the
//! generation, so the frame stamp is what invalidates; invariant #15),
//! joins the two by source-relative path, and returns
