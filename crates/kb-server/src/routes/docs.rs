...
pub async fn list(
    ...
    // each re-running both.
    //
    // PF-F1 — the atlas projection is the AtlasView's WIDER scan
    // (`list_docs_with_atlas`, ~30µs/doc measured, a 150-200ms tax at
    // prod-corpus scale) and used to be served fully uncached on every
    // request ("P3 owns atlas perf" — the deferral this fixes). It now
    // shares `routes::atlas::atlas_docs_cached` — the SAME per-(kb,
    // generation, atlas-frame) memo `GET /atlas/points` and
    // `atlas_field::disagreement` already warm (one memo, many consumers,
    // rather than a second full scan). That memo returns raw
    // `Arc<Vec<DocSummary>>` (it's also consumed by `build_atlas_points_
    // response`, which wants the bare rows), so the docs.rs pipeline below
    // — which needs `DocRow`'s `folder` field for `docs_query::matches` —
    // decorates a CLONE of the cached rows rather than the `GalleryCache`
    // shape's zero-clone borrow: cloning `DocSummary` structs is a plain
    // in-memory copy (no lance I/O, no datafusion), nowhere near the scan
    // this replaces. `edge_counts()` stays a fresh per-request read, same
    // as before — it's a separate, already-cheap aggregate unrelated to
    // atlas coordinates.
    let (rows, edge_counts) = if projection == Projection::Atlas {
        let generation = ctx.storage.index_generation();
        let raw = match crate::routes::atlas::atlas_docs_cached(ctx, generation).await {
            Ok(r) => r,
            Err(e) => return error_to_problem_json(&e),
        };
        let decorated = Arc::new(decorate_rows_borrowed(raw.as_slice(), &ctx.source_path));
...
/// PF-F1 — like `decorate_rows` above but over a BORROWED slice: the atlas
/// projection's row set comes back `Arc`-shared from `routes::atlas::
/// atlas_docs_cached` (invariant #15 — the memo's own cache slot holds
/// another clone of that `Arc`, so the docs here can't be moved out of it).
/// One `DocSummary` clone per row — see the call site's comment for why
/// that's an acceptable cost.
fn decorate_rows_borrowed(
    rows: &[kb_core::storage::lance::DocSummary],
    source_root: &std::path::Path,
) -> Vec<DocRow> {
    rows.iter()
        .map(|doc| {
