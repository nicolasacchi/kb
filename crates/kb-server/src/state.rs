...
pub struct KbContext {
    ...
    /// `gallery_cache`/`facets_cache` (invariant #15) — **plus** a second key
    /// (`atlas_frame_id`, PF-F1) since `update_atlas` writes don't bump the
    /// generation at all; see [`AtlasPointsCache`]'s doc comment. Shared by
    /// three consumers of `routes::atlas::atlas_docs_cached`: `GET
    /// /atlas/points`, `routes::atlas_field`'s `disagreement` route, and (PF-F1)
    /// `GET /docs?projection=atlas`'s row set.
...
/// M-a — a per-(kb, index-generation, atlas-frame) snapshot of the
/// FULL-corpus atlas point set (`list_docs_with_atlas(u32::MAX)`), so `GET
...
/// request. Same single-slot discipline as [`GalleryCache`]/[`FacetsCache`]
/// (invariant #15): `docs` is `Arc`-shared so a hit is one clone.
///
/// PF-F1 — **two keys, not one.** `generation` alone is stale-blind to a
/// recompute/recluster: `update_atlas`/`set_atlas_labels` writes deliberately
/// do NOT bump the storage actor's index generation (invariant #15 — they
/// touch no row-set/edge, verified by
/// `index_generation_bumps_only_on_row_set_and_edge_mutations`), so a memo
/// keyed on `generation` alone would keep serving pre-recompute
/// `atlas_x`/`atlas_y`/`atlas_cluster` forever, across every consumer of this
/// cache, until an UNRELATED doc write happened to bump the generation for
/// some other reason. `atlas_frame_id` — the newest row in the `atlas_snapshots`
/// time-lapse table (W3 T-a) — is the one stamp that DOES change exactly when
/// the live columns do: a frame write dedups on a coord-hash of the full
/// point set (`Db::atlas_frame_insert`), so an unchanged layout keeps the
/// same id and any actual coordinate/cluster change always gets a new one.
/// `None` before a kb's first recompute (every row's atlas fields are also
/// `None` then, so there's nothing to go stale). A hit requires BOTH
/// `generation == storage.index_generation()` AND `atlas_frame_id` to match
/// the freshly-read newest frame id — see
/// `routes::atlas::atlas_cache_is_fresh` for the pure two-key predicate and
/// `routes::atlas::atlas_docs_cached` for the read-then-compare sequence.
pub struct AtlasPointsCache {
    pub generation: u64,
    pub atlas_frame_id: Option<i64>,
