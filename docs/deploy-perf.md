...
- **Ingest throughput degrades sharply with corpus size** (BM25-only,
  HDD host): ~290 docs/min while corpora were small → ~21 docs/min/kb
  past ~5k docs. Suspects: lance merge_insert cost growing with dataset
  size + fragment accumulation vs the auto-compact cadence. Needs its
  own investigation before any bulk-import advice.
- **Daemon core RSS is small**: the lab daemons idled at 186MB (2k docs)
  and 282MB (~14k docs) — the prod container's multi-GiB RSS is not
  baseline daemon weight; the remaining suspects are the ORT embedder
  sidecar, 24 corpora of lance mmap, and long uptime.
