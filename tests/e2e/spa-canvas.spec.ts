...
test.describe("reflection canvas", () => {
  ...
  test("the KEYBOARD brush narrows the window and pivots to a gallery ids= filter", async ({
    ...
    // Count /timeline requests from here on: brushing must trigger NONE.
    let timelineCalls = 0;
    page.on("request", (r) => {
    ...
    // End on the START handle collapses the brush onto the window's last UTC
    // day — the day the fixture corpus was written.
    await start.focus();
    await start.press("End");
    await expect(start).toHaveAttribute("aria-valuetext", lastDay!);
    expect(await start.getAttribute("aria-valuetext")).not.toBe(wholeWindowStart);
    await expect(page.locator(".rcanvas__summary")).toContainText("1 day");

    // Widen it again by a week from the start handle (arrows + PageDown are
    // the same handler; PageDown is the 7-day step).
    ...
    const fromInput = page.locator('.rcanvas__date input[type="date"]').first();
