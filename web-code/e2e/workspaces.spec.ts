...
test.describe("workspaces", () => {
  test("save, list under the branch, open + restore, notes persist", async ({ page, context }) => {
    ...
    // --- open → restore ----------------------------------------------------
    await openBtn.click();
    // `[?&]` rather than `?`: spec DRIFT from V70-A6's Location Contract
    // (`nav/location.ts`, shipped in this same v7.0 chain and extended
    // through wave 2), which CANONICALISES query order on every URL it
    // emits — every param comes out of `lib/codeUrl.ts`'s own builders, in
    // their order, so the restored reader lands on
    // `…/caller.rs?line=5&workspace=set_…` and `workspace=` is no longer
    // guaranteed to be the FIRST param. What the assertion pins is
    // unchanged: the restore is addressed by a `set_` id in the URL.
