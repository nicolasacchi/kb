...
describe("scope goldens", () => {
  ...
  it("global", () => {
    expect(table("global")).toEqual([
      ...
      "Space R f → rail.tab.facts",
      "Space R g → facts.coverage-band",
      ...
      "Space k r → tour.record",
      "Space k s → tour.record-stop",
  ...
  it("branches", () => {
    expect(table("branches")).toEqual([
      ...
      "F → hint.act",
      "H → branches.view-prev",
      "K → peek.hover",
      "L → branches.view-next",
      "Meta-k → cmd.palette.search",
      "O → ramp.open-window",
      "P → branches.favourite",
      "R → branches.radar",
      "S → scope.clear",
      "Shift-Enter → ramp.open-pane2",
      "Shift-F10 → action.panel",
