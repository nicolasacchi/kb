...
/// and stops EVERY keydown from bubbling (mirrors `KeyboardHelp`'s
/// Esc-capture, but for the whole key set, since this panel — unlike that
/// static cheatsheet — has rows to navigate) so neither the vim keymap nor
/// the window-level tree-nav handler double-handles Up/Down/j/k/Enter/Esc
/// while it's up.
///
...
export default function PeekPanel({
  ...
  function onKeyDown(e: React.KeyboardEvent) {
    // Stop EVERY key here from reaching the vim keymap / window-level tree
    // handler — see the module doc. `preventDefault` only for the keys this
    // panel actually acts on, so e.g. Tab still behaves natively.
    e.stopPropagation();
    switch (e.key) {
      case "ArrowDown":
      case "j":
