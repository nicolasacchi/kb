...
export default function SearchSection({
      ...
      case "files":
        return (section.results as FileHit[]).map((hit, i) =>
          readerRow(
            i,
            basename(hit.path),
            `${hit.repo} · ${hit.path}${typeof hit.score === "number" ? ` · ${hit.score.toFixed(0)}` : ""}`,
          ),
        );
      case "symbols":
  ...
  return (
    <section
      ...
      role="group"
      aria-label={LANE_LABELS[section.lane]}
    >
      <Link
        ...
        onClick={handleNavClick}
      >
        <span className="kbc-search__header-label">{LANE_LABELS[section.lane]}</span>
