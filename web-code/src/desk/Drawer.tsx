...
export interface DrawerProps {
  ...
  overlay?: boolean;
  /// V71-E2 — a set whose OWNER renders its own body (the Usages dock:
  /// census → chips → grouped tree → CM6 preview). Returning `null` keeps
  /// the generic trust-grouped row list, which is still the right rendering
  /// for every other kind. A SLOT rather than a `kind` switch here on
  /// purpose: `drawerSets.ts` stays a pure reducer with no knowledge of any
  /// wire type, and the host that fetched the rows is the one that knows
  /// how to render them.
  renderBody?: (setId: string) => React.ReactNode | null;
}
...
export default function Drawer({
  ...
  const customBody = active && renderBody ? renderBody(active.id) : null;
  ...
  function onKeyDown(e: React.KeyboardEvent) {
    if (!active) return;
    // A set with its own body owns its own keys (the Usages dock walks with
    // `]u`/`[u` through the command registry, not through this reducer's
    // row cursor, which does not index its grouped tree).
    ...
    >
          ...
          return (
            ...
            >
              ...
              >
                ...
                <span className="kbc-desk__drawer-tab-count">{drawerSetCount(s)}</span>
      ...
      {!collapsed && (
        <div
          ...
          ref={bodyRef}
          tabIndex={0}
          role={customBody ? undefined : "listbox"}
        ...
        >
          ...
          {!active ? (
            <EmptyState
              ...
              hint="Press gr on a symbol — the classified usages dock opens here, beside the code."
            />
          ) : customBody ? (
            customBody
