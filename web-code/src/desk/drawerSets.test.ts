...
// --- V71-E2: a set whose owner renders its own body ------------------------

describe("drawerSetCount (V71-E2)", () => {
  it("defaults to rows.length, byte-identical to every pre-E2 set", () => {
    const st = drawerSetsReducer(initialDrawerSets, {
      type: "keep",
      set: { id: "usages:a", title: "a", kind: "usages", rows: rows(2) },
    });
    expect(drawerSetCount(st.sets[0])).toBe(2);
  });

  it("an explicit count wins — the badge is the SERVER's total, never rows.length", () => {
    // The Usages dock keeps `rows` empty (its body is a `usages/2` payload),
    // so a badge derived from `rows.length` would read 0 beside a census
    // reading 1,841 — the disagreeing-count bug this milestone is about.
    const st = drawerSetsReducer(initialDrawerSets, {
      type: "keep",
      set: { id: "usages:a", title: "a", kind: "usages", rows: [], count: 1841 },
    });
    expect(drawerSetCount(st.sets[0])).toBe(1841);
  });

  it("re-keeping the same id refreshes the count in place", () => {
    let st = drawerSetsReducer(initialDrawerSets, {
      type: "keep",
      set: { id: "usages:a", title: "a", kind: "usages", rows: [], count: 3 },
    });
    st = drawerSetsReducer(st, {
      type: "keep",
      set: { id: "usages:a", title: "a", kind: "usages", rows: [], count: 9 },
    });
    expect(st.sets).toHaveLength(1);
    expect(drawerSetCount(st.sets[0])).toBe(9);
  });
});
