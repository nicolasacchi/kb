...
describe("placement rule table", () => {
  ...
  it("the shipped census is exactly the rows a consumer reads today", () => {
    ...
    expect(shipped).toEqual(
      // V71-E2 adds `usages`: `routes/Reader.tsx`'s `handleFindRefs` now
      // fetches `usages/2` and hands the drawer its OWN body (the Usages
      // dock), which is exactly the consumer this row promised.
      ["annotation", "blame", "definition-single", "file-open", "peek", "review-thread", "symbol-info", "usages"].sort(),
    );
  });
  ...
  it("the printable table has one line per rule and flags the unrouted ones", () => {
    ...
    // V71-E2 routed it: the line no longer carries the unrouted marker.
    expect(lines.some((l) => l.startsWith("usages → drawer:usages —"))).toBe(true);
    expect(lines.some((l) => l.includes("(declared, not yet routed)"))).toBe(true);
  });
});
