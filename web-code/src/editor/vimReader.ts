...
export interface VimReaderCallbacks {
  ...
  /// V71-K4 — the ONE thing this layer asks the outside world BEFORE it
  /// interprets a key: "does `CommandRoot`'s in-flight chord consume this
  /// keystroke?". Not an `on*` callback — nothing is being reported; it is
  /// a question, asked of the central dispatcher through the same
  /// callbacks channel every `cb-*` arm already uses (`CodeView` fills it
  /// from the command bus). Absent ⇒ always `false`, i.e. exactly the
  /// pre-V71-K4 behaviour, so a host with no dispatcher around it (a bare
  /// `CodeView` in a story) is unaffected.
  ///
  /// Why it must exist at all: this keymap is installed at `Prec.highest`
  /// INSIDE the buffer, so it sees every keystroke before the window
  /// listener does — and `vimKeysReducer` evaluates each one
  /// independently, with no memory that a `Space`-prefixed central chord
  /// is under way. Without the question, a bare letter that is both some
  /// chord's final token and a standalone vim key fires BOTH layers:
  /// `Space u` ran `drawer.reopen` AND `nav.back` (which unmounted the
  /// reader), `Space R a` ran `rail.tab.all` AND `annotate.line`. See
  /// `commands/CommandRoot.tsx`'s `chordConsumesKey` for what "consume"
  /// means and why `[c`/`Ctrl-w v` and friends are deliberately NOT
  /// consumed.
  chordWillConsume?(event: KeyboardEvent): boolean;
}
...
function makeKeydownHandler(cb: VimReaderCallbacks) {
  return (event: KeyboardEvent, view: EditorView): boolean => {
    // V71-K4 — asked FIRST, before this layer forms any opinion at all: a
    // central chord already in flight owns its own continuation keystroke.
    // Returning false leaves the event untouched and un-`preventDefault`ed,
    // so it finishes bubbling to `CommandRoot`'s window listener and is
    // handled there EXACTLY once (guard 2 lets it straight through — its
    // own `chordPendingLength !== 0` bail).
    //
    // The reducer is deliberately NOT fed the key. Feeding it "for
    // bookkeeping" and dropping the commands would be worse than not
    // asking: `v`/`V` carry the mode in the STATE as well as in a command,
    // so `Space P v` would leave the buffer believing it is in visual mode
    // while nothing reshaped the selection — a status line that lies. What
    // IS cleared is a half-typed vim sequence, which is the one thing the
    // skipped reducer call would legitimately have done: `[` sets
    // `pendingPrefix` before the second key decides whose the chord is
    // (`[d` is the drawer's, `[c` is vim's), and a stranded prefix would
    // silently swallow the NEXT keystroke as `[<whatever>`. Mode is
    // preserved — abandoning a sequence is not leaving visual mode.
    if (cb.chordWillConsume?.(event)) {
      const held = view.state.field(vimStateField);
      if (held.pendingCount !== "" || held.pendingPrefix !== "") {
        view.dispatch({ effects: setVimState.of({ ...held, pendingCount: "", pendingPrefix: "" }) });
      }
      return false;
    }
