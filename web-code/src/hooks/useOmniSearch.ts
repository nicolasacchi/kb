...
const EMPTY_STATE: OmniSearchState = { sections: [], queryEcho: "", loading: false, error: null };
    ...
    const timer = setTimeout(() => {
      ...
      fetchSearch({ q, repo, limit, signal: ctl.signal })
        .then((resp) => {
          ...
          setState({ sections: resp.sections, queryEcho: resp.query_echo, loading: false, error: null });
        ...
        .catch((e) => {
          ...
          setState({ sections: [], queryEcho: "", loading: false, error: String(e) });
        });
    }, DEBOUNCE_MS);
    return () => {
      clearTimeout(timer);
