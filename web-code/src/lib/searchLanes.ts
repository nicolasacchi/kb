...
/// Best-effort extraction of a `repo:<name>` filter token from a raw query
/// string - mirrors `grammar::extract_filters`'s `repo:` key loosely (last
/// occurrence wins, whitespace-separated tokens) for the ONE place the SPA
/// needs to know which repo a filter-scoped hit resolved against: the text
/// lane's row targets carry no `repo` field of their own (`search::text`'s
/// single-repo-per-request design - see `search::unified`'s module doc), so
/// building a `readerUrl` for a text-lane hit needs this to recover the
/// repo when the search itself isn't already route-scoped to one. This is
/// deliberately a small UI-only re-implementation, not a full grammar
/// parse - the daemon's `grammar::parse` stays the single source of truth
/// for what a query actually MEANS.
export function extractRepoFilter(query: string): string | undefined {
  let found: string | undefined;
  for (const tok of query.trim().split(/\s+/)) {
    const m = /^repo:(.+)$/.exec(tok);
    if (m && m[1]) found = m[1];
  }
  return found;
}
