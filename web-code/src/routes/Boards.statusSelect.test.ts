...
// V76-R4d.2 — pin for the boards list `status` <select>. No shipped spec
...
// `Boards.tsx` is local OPTIMISTIC state: the select flips synchronously
...
const BOARDS_LIST = {
  schema: "boards/1",
  repo: "demo",
  statuses_available: ["pending", "accepted"],
  boards: [],
};
...
describe("Boards status-select (react-router 7 startTransition, V76-R4d.2)", () => {
  beforeEach(() => {
    lastSearch = "";
    globalThis.fetch = (async () =>
      new Response(JSON.stringify(BOARDS_LIST), {
        status: 200,
  ...
  it("select flips SYNCHRONOUSLY on change (no waitFor window)", async () => {
    ...
    await settle();
    const sel = container.querySelector<HTMLSelectElement>("[data-kbc-boards-status]");
    expect(sel).toBeTruthy();
    expect(sel!.value).toBe("");
    // Pre-fix this assertion fails: the select stays at "" until the
    // router's transition commits.
    pickOption(sel!, "pending");
    expect(sel!.value).toBe("pending");
  });
  ...
  it("optimistic state reconciles with the URL once the navigation lands", async () => {
    ...
    expect(sel.value).toBe("pending");
    await settle();
    expect(lastSearch).toContain("status=pending");
